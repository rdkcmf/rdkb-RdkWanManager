...
static void generate_client_duid_conf()
{
    ...
    if( fp_duid == NULL )
    {
        if (syscfg_get(NULL, SYSCFG_WAN_INTERFACE_NAME, wan_phy_interface, sizeof(wan_phy_interface)) != ANSC_STATUS_SUCCESS)
        {
            CcspTraceError(("Failed to get PHY WAN interface name \n"));
            return ANSC_STATUS_FAILURE;
        }
        sprintf(file_path, "/sys/class/net/%s/address", wan_phy_interface);
...
static void createDummyWanBridge()
{
    ...
    snprintf(syscmd, sizeof(syscmd), "brctl addbr %s", wan_phy_interface);
    system(syscmd);

    memset(syscmd, '\0', sizeof(syscmd));
    snprintf(syscmd, sizeof(syscmd), "ip link set dev %s address %s", wan_phy_interface, wan_mac);
    system(syscmd);
...
static void deleteDummyWanBridgeIfExist()
{
    ...
    memset(syscmd, '\0', sizeof(syscmd));
    snprintf(cmd, sizeof(cmd), "ip -d link show %s | tail -n +2 | grep bridge", wan_phy_interface);
    fp = popen(cmd, "r");
    if (fp != NULL)
    {
        ...
        if (resultBuff[0] == '\0')
        {
            ...
            CcspTraceInfo(("%s brdige interface is not exists in the system \n", wan_phy_interface));
        }
        else
        {
            CcspTraceInfo(("%s bridge interface is found in the system, so delete it \n", wan_phy_interface));
            /* Down the interface before we delete it. */
            snprintf(syscmd, sizeof(syscmd), "ifconfig %s down", wan_phy_interface);
            system(syscmd);
            memset(syscmd, '\0', sizeof(syscmd));
            snprintf(syscmd, sizeof(syscmd), "brctl delbr %s", wan_phy_interface);
            system(syscmd);
        }
        pclose(fp);
    }
