    ...
    )
{
    ...
    {
        strcpy(buf, SYSCFG_FORMAT_DHCP6C"_alias");
        syscfg_set_string(buf, pCfg->Alias);
    }
    ...
    {
        strcpy(buf, SYSCFG_FORMAT_DHCP6C"_requested_options");
        syscfg_set_string(buf, pCfg->RequestedOptions);
        need_to_restart_service = 1;
    }
    ...
    *pSize = 0;
    if (fp)
    {
        ...
        *ppCfg = (PDML_DHCPCV6_SVR)AnscAllocateMemory( sizeof(DML_DHCPCV6_SVR) );
        ...
        strcpy((*ppCfg)->InformationRefreshTime, "0001-01-01T00:00:00Z");
        ...
        while(fgets(buf, sizeof(buf)-1, fp))
        {
            ...
            {
                strcpy((*ppCfg)->SourceAddress, val);
                entry_count |= 1;
            }
    ...
    )
{
    ...
    sprintf(namespace, SYSCFG_DHCP6C_SENT_OPTION_FORMAT, ulIndex+1);
    snprintf(out, sizeof(out), "%d", ulInstanceNumber);
...
static int _syscfg_add_sent_option(PDML_DHCPCV6_SENT      pEntry, int index)
{
    ...
    snprintf(out, sizeof(out), "%d", pEntry->InstanceNumber);
    syscfg_set_string(namespace, out);
    ...
    )
{
    ...
    if (fp)
    {
        while (fgets(buf, sizeof(buf)-1, fp))
        {
            ...
            strcpy(p_rcv->Server, "Device.DHCPv6.Client.1.Server.1");
