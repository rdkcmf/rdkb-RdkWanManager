...
int UtilGetIfStats(char * ifname, PDML_IF_STATS  pStats);

ULONG UtilIoctlXXX(char * if_name, char * method, void * input);
ULONG NetmaskToNumber(char *netmask);
ANSC_STATUS
UtilGetStaticRouteTable
    (
        UINT                        *count,
        StaticRoute                 **out_route
    );

int UtilGetIpv6AddrInfo (char * ifname, ipv6_addr_info_t ** pp_info, int * num);
...
int DmlV6AddrIsEqual(char * p_addr1, char * p_addr2);
int DmlV6PrefIsEqual(char * p_pref1, char * p_pref2);
int _write_sysctl_file(char * fn, int val);

/*utility functions for linktype and lowerlayer*/
char*               UtilGetLinkTypePath(DML_LINK_TYPE LinkType);
char*               UtilGetStrFromLinkTypePath(char* pLinkTypePath);
char*               UtilGetLinkTypeStr(DML_LINK_TYPE LinkType);
DML_LINK_TYPE  UtilGetLinkTypeFromStr(char* pLinkTypeStr);
DML_LINK_TYPE  UtilGetLinkTypeFromPath(char*pLinkTypePath);

// utility functions for getting Bridge information
PUCHAR UtilFindBridgeName(char* pBridgePath);
PUCHAR UtilFindBridgePath(char* pBridgeName);
