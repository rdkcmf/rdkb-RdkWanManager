...
{
    if(iface_index < gWanMgrDataBase.IfaceCtrl.ulTotalNumbWanInterfaces)
    {
        if(gWanMgrDataBase.IfaceCtrl.pIface != NULL)
        {
            WanMgr_Iface_Data_t* pWanIfaceData = &(gWanMgrDataBase.IfaceCtrl.pIface[iface_index]);

            //lock
            if(pthread_mutex_lock(&(pWanIfaceData->mDataMutex)) == 0)
            {
...
{
   UINT totalIfaces = gWanMgrDataBase.IfaceCtrl.ulTotalNumbWanInterfaces;
   UINT idx;

    if(gWanMgrDataBase.IfaceCtrl.pIface != NULL)
    {
        for(idx = 0; idx < totalIfaces; idx++)
        {
            WanMgr_Iface_Data_t* pWanIfaceData = &(gWanMgrDataBase.IfaceCtrl.pIface[idx]);
            ...
            if(pthread_mutex_lock(&(pWanIfaceData->mDataMutex)) == 0)
            {
                ...
                pthread_mutex_unlock (&(pWanIfaceData->mDataMutex));
            }
        }
    }
...
void WanMgr_IfaceData_Init(WanMgr_Iface_Data_t* pIfaceData, UINT iface_index)
{
    ...
    if(pIfaceData != NULL)
    {
        ...
        pthread_mutex_init(&(pIfaceData->mDataMutex), &(muttex_attr));
    }
}
...
ANSC_STATUS WanMgr_Data_Delete(void)
{
    ...
    /*** WAN IFACE ***/
    for(idx = 0; idx < pWanMgrData->IfaceCtrl.ulTotalNumbWanInterfaces; idx++)
    {
        WanMgr_Iface_Data_t* pIfaceData = &(pWanMgrData->IfaceCtrl.pIface[idx]);
        pthread_mutex_destroy(&(pIfaceData->mDataMutex));
    }
