...
/***************************************************************************
 * @brief API used to check the existence of a file in system
 * @param filename contains the name of file
 * @param returns true if file exists else returned false
 ****************************************************************************/
static bool util_isFilePresent(char *filename);

/***************************************************************************
 * @brief API used to launch client applications
 * @param appName Name of the application
 * @param cmd Commandline arguments
 * @return pid of the launched application.
 ****************************************************************************/
static int LaunchApp(const char *appName, const char *cmd);
 ...
 * @return None
 ****************************************************************************/
static void CollectApp(int pid);
 ...
 * @return pid of the launched application.
 ****************************************************************************/
static int GetPathToApp(const char *name, char *buf, uint32_t len);
...
/***************************************************************************
 * @brief API to return the path to the "root" directory in the given buffer.
 * @param pathBuf Contains the pathBuf.
 * @param pathBufLen Length of the buffer.  If the buffer is not large
 ...
 * @param plen hold the prefix length
 * @param pltime prefered life time
 * @param vltime valid life time
 * @return TRUE if IAPD prefix is read from file else returned false.
 ****************************************************************************/
...
static void CollectApp(int pid)
{
    ...
    {
        CcspTraceInfo(("collected pid %d", pid));
    }
    return;
}
...
{
    if (WanManager_DoStartApp(BFD_CLIENT_NAME, "") == CMS_INVALID_PID) // no command line args
    {
        CcspTraceError(("could not start %s", BFD_CLIENT_NAME));
...
{
    WanManager_DoStopApp(BFD_CLIENT_NAME);
...
void WanManagerInitInterfaceData(WanData_t *wanData, const char *ifname)
{
    ...
    wanData->isIPv4ConfigChanged = FALSE;
    wanData->isIPv6Up = FALSE;
    wanData->isIPv6ConfigChanged = FALSE;
    wanData->isMAPTUp = FALSE;
    wanData->isMAPTConfigChanged = FALSE;
    memset(wanData->baseIfName, 0, sizeof(wanData->baseIfName));
    memset(wanData->ifName, 0, sizeof(wanData->ifName));
    /* Initialise the interface name. */
    strncpy(wanData->ifName, ifname, sizeof(wanData->ifName));

    ipv4Info_init(&wanData->ipv4Data, ifname);
    ipv6Info_init(&wanData->ipv6Data);
    memset(wanData->wanConnectionData.ifName, 0, sizeof(wanData->wanConnectionData.ifName));
    strncpy(wanData->wanConnectionData.ifName, ifname, sizeof(wanData->wanConnectionData.ifName)); //Initialise wan interface name.
    memset(wanData->wanConnectionData.ipv4Address, 0, sizeof(wanData->wanConnectionData.ipv4Address));
    memset(wanData->wanConnectionData.ipv6Address, 0, sizeof(wanData->wanConnectionData.ipv6Address));
}
...
int WanManager_AddDefaultGatewayRoute(const dhcpv4_data_t *ipv4Info)
{
   ...
   if (IsValidIpv4Address(ipv4Info->gateway) && !(IsZeroIpvxAddress(AF_SELECT_IPV4, ipv4Info->gateway)))
   {
       snprintf(cmd, sizeof(cmd), "route add default gw %s dev %s", ipv4Info->gateway, ipv4Info->dhcpcInterface);
       WanManager_DoSystemAction("SetUpDefaultSystemGateway:", cmd);
